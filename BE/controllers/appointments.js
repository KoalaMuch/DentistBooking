...
exports.getAppointment = async (req, res, next) => {
    try {
        ...
        if (!appointment) {
            return res.status(404).json({success: false, message: `Cannot find Appointment with id ${req.params.id}`});
        }
...
exports.addAppointment = async (req, res, next) => {
    try {
        ...
        if(req.user.role !== 'admin' && existedAppointments.length >= 3) {
            return res.status(400).json({success: false, message: 'The user with ID ' + req.user.id + ' has already booked 3 appointments'});
        }
